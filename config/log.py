...
class MyLogger:
    def __init__(self):
        self.logger = logger
        self.logger.remove()
        self.logger.add(sys.stderr, level="INFO", format=" <green>[{level}]</green>: <cyan>{time:YYYY-MM-DD HH:mm:ss} | {module} |</cyan> <green>>> {message}</green>", filter=self.no_error)
        self.logger.add(sys.stderr, level="ERROR", format=" <red>[{level}]</red>: <cyan>{time:YYYY-MM-DD HH:mm:ss} | {module} |</cyan> <red>>> {message}</red>")
